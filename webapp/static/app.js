...
const apiPermsSet = (gid,granteeId,canManage)=>api('/api/webapp/friends/groups/'+gid+'/permissions/'+granteeId,{method:'PUT',body:JSON.stringify({can_manage:!!canManage})});
...
// Bu tarmoq "storm"ini oldini oladi — 5 ta reja ketma-ket belgilансa 10 emas,
// 2 ta so'rov ketadi. UI baribir darhol (local state + xpPop) yangilanadi.
...
async function openGroup(gid){
  ...
  document.getElementById('friendsGroupSettings').style.display=d.is_owner?'':'none';
  renderGroupMembers();
}
  ...
  el.innerHTML=(g.members||[]).map(m=>{
    ...
    return `<div class="mem-card ${m.is_me?'me':''}" data-uid="${m.user_id}">
      ...
      <div class="body">
        ...
        <div class="st">${_sumChips(m.summary||{})} <span class="chip">🔥 ${m.streak||0}</span></div>
      </div>
...
async function openPermsModal(){
  ...
  const out=document.getElementById('permsGrantsOut');
  const inp=document.getElementById('permsGrantsIn');
  out.innerHTML=(d.grants_out||[]).length?(d.grants_out.map(m=>`
    <div class="perm-row">
      <div class="nm">${esc(m.name)}</div>
      <div class="toggle ${m.can_manage?'on':''}" data-uid="${m.user_id}"><i></i></div>
    </div>`).join('')):'<div style="font-size:12px;color:var(--text-3);padding:6px">Guruhda boshqa a\'zo yo\'q</div>';
  inp.innerHTML=(d.grants_in||[]).filter(m=>m.can_manage).length?d.grants_in.filter(m=>m.can_manage).map(m=>`
    <div class="perm-row"><div class="nm">${esc(m.name)}</div><span class="chip mine" style="padding:2px 8px;font-size:10.5px;font-weight:800;color:#fff;background:var(--grad);border-radius:999px">🛡 huquqli</span></div>`).join(''):'<div style="font-size:12px;color:var(--text-3);padding:6px">Hech kim sizga ruxsat bermagan</div>';
  out.querySelectorAll('.toggle').forEach(t=>t.onclick=async()=>{
    const uid=+t.dataset.uid;const nv=!t.classList.contains('on');
    t.classList.toggle('on',nv);
    try{await apiPermsSet(g.id,uid,nv);toast(nv?'🛡 Ruxsat berildi':'Ruxsat olindi');}
    catch(e){t.classList.toggle('on',!nv);toast('Xato: '+e.message,true);}
  });
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  // Boshlang'ich yuklamalar — o'zaro bog'liq bo'lmagan so'rovlarni PARALLEL
  // yuboramiz (avval loadPlansAPI().then(...) waterfall edi — 4 so'rov ketma-ket).
  // Har biri o'z bo'limini mustaqil render qiladi, shuning uchun parallel xavfsiz.
  loadSubscription();
  loadProfileMeta();
