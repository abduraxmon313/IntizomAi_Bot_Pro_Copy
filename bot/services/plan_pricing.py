...
def _merge_overrides(overrides: list[SubscriptionPlanOverride]) -> dict:
    ...
    for ov in overrides or []:
        if ov.plan_key in merged and ov.price is not None and ov.price >= 0:
...
async def set_plan_price(
    ...
    """
    Tarif narxini o'zgartiradi (yoki yangi override yaratadi).
    Keshni ham yangilaydi — keyingi barcha o'qishlar yangi narxni ko'radi.
    """
    ...
    if row is None:
        row = SubscriptionPlanOverride(
            plan_key=plan_key, price=price, updated_by=updated_by,
        )
        session.add(row)
    else:
        row.price = price
...
async def reset_plan_price(session: AsyncSession, plan_key: str) -> None:
    """Override'ni o'chiradi — narx `bot/config.py`'dagi default'ga qaytadi."""
    row = await session.get(SubscriptionPlanOverride, plan_key)
    if row is not None:
